...
#echo "${STATUS}"
PARENT_SOURCE=`echo ${STATUS} | jq --raw-output '.result | select(.reference=="'"${CONTAINER_REFERENCE}"'" and .namespace==null) | .provisionContainer '`
...
elif [[ "${CONTAINER_TYPE}" == "MSSqlDatabaseContainer" ]]
then
   # MSSqlExistingMostRecentBackupSyncParameters
   # MSSqlExistingSpecificBackupSyncParameters
   # MSSqlNewCopyOnlyFullBackupSyncParameters
   ...
   REFRESH_TYPE="RefreshParameters"
   ROLLBACK_TYPE="RollbackParameters"
elif [[ "${CONTAINER_TYPE}" == "ASEDBContainer" ]]
then
   # ASELatestBackupSyncParameters
   # ASENewBackupSyncParameters
   # ASESpecificBackupSyncParameters 
   ...
   REFRESH_TYPE="RefreshParameters"
   ROLLBACK_TYPE="RollbackParameters"
elif [[ "${CONTAINER_TYPE}" == "AppDataContainer" ]]
then
   SYNC_TYPE="AppDataSyncParameters"
   REFRESH_TYPE="RefreshParameters"
   ROLLBACK_TYPE="RollbackParameters"
else
   echo "Container Type: ${CONTAINER_TYPE} code not included yet, trying default options ..."
fi
